...
Evaluate emotion2vec at the full emotion level (not 3-way valence).

Reuses existing per-corpus predictions.csv from results/emotion2vec/
(ground-truth emotion parsed from cleaned filenames; prediction =
emotion2vec native_label). Writes confusion-matrix PNGs under:

  results/emotion2vec/result_diagrams/confusion_matrix_all_emotions/

Usage:
    uv run python emotion2vec_all_emotions.py
    uv run python emotion2vec_all_emotions.py --corpus tess_human_cleaned
    uv run python emotion2vec_all_emotions.py --dry-run
...
# Preferred label order on confusion matrices.
PREFERRED_ORDER = (
    "angry",
    "disgusted",
    "fearful",
...
def load_predictions(corpus: str) -> pd.DataFrame:
    ...
    df["pred_emotion"] = df["native_label"].map(normalise_emotion)
    return df
...
def compute_metrics(df: pd.DataFrame) -> dict:
    ...
    Confusion matrices use GT ∪ predicted labels so model-only classes
    (surprised / other / unknown) still appear as prediction columns.
    Scalar metrics / the classification report use GT emotion labels only,
    which avoids sklearn's ``y_pred contains classes not in y_true`` warning
    and zero-support rows for labels the corpus never has.
    ...
    return {
        ...
        # Mean recall over GT emotions only (balanced accuracy without pred-only classes).
...
def plot_confusion_matrices(corpus: str, metrics: dict) -> None:
    ...
    ax.set_title(f"{corpus} emotion confusion matrix (counts)")
    ...
    ax.set_xlabel("Predicted emotion")
    ax.set_ylabel("True emotion")
    ax.set_title(f"{corpus} emotion confusion matrix (row-normalised)")
    plt.xticks(rotation=45, ha="right")
    plt.yticks(rotation=0)
    fig.tight_layout()
...
def main() -> None:
    ...
    print(f"[all-emotions] human-GT filenames = {len(human_gt_files)} from {args.heet}")
    ...
    print(f"[all-emotions] diagrams -> {DIAGRAMS_ROOT}")
    print(f"[all-emotions] metrics  -> {METRICS_ROOT}")
