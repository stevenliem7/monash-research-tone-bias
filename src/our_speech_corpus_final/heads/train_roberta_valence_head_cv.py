...
Frozen Cardiff Twitter-RoBERTa encoder (discard classification head) + valence
head CV — same protocol as DistilBERT / MiniLM / WavLM grids.

Model: ``cardiffnlp/twitter-roberta-base-sentiment-latest`` loaded as
``AutoModel`` (768-d). Mean-pool is primary; CLS is a robustness check.

Native 3-class sentiment-tuned encoder → another hard text baseline (alongside
DistilBERT-SST2). MiniLM remains the fair general-purpose text comparison.

Usage:
    uv run python src/our_speech_corpus_final/heads/train_roberta_valence_head_cv.py \\
        --manifest data/our_speech_corpus_manifest_split_80_20.csv \\
        --group-by question --pooling mean --compare-wavlm \\
        --output data/our_speech_corpus_text/reports/valence_head_cv_roberta_group_question.json
"""
...
def extract_roberta_embeddings(
    frame,
    output_path: Path,
    model_id: str = TWITTER_ROBERTA_SENTIMENT_MODEL_ID,
    batch_size: int = 32,
    max_length: int = 128,
    ...
    Args:
        ...
        model_id: HF Twitter-RoBERTa sentiment id (loaded as AutoModel).
    ...
    # AutoModel loads the encoder without the 3-class classification head.
    ...
    print(f"[wrote] {output_path} mean={mean_emb.shape} cls={cls_emb.shape}")
    return {
...
def main() -> None:
    """CLI for RoBERTa encoder extract + matched valence-head CV.
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Frozen Twitter-RoBERTa encoder valence-head CV "
            "(3-class sentiment-tuned text baseline)"
        )
    )
    parser.add_argument("--manifest", type=Path, default=SPLIT_MANIFEST_CSV)
    parser.add_argument("--embeddings", type=Path, default=ROBERTA_EMBEDDINGS_NPZ)
    parser.add_argument("--output", type=Path, default=ROBERTA_CV_GROUP_QUESTION_JSON)
    parser.add_argument("--model-id", default=TWITTER_ROBERTA_SENTIMENT_MODEL_ID)
    ...
    if not args.skip_extract:
        extract_meta = extract_roberta_embeddings(
            frame,
            args.embeddings,
    ...
    results = evaluate_embedding_grid(
        ...
        y=y,
        frame=frame,
        modality=f"text_roberta_{args.pooling}",
        ...
        extra_meta={
            ...
            "comparison_note": (
                "Hard text baseline: Twitter-RoBERTa is 3-class sentiment-tuned; "
                "WavLM is general-purpose. MiniLM is the fair comparison."
            ),
        },
    )
